...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists("Users/628135715/phone.csv"):
    os.mkdir('./Users')
    os.mkdir('./Users/628135715')
    open("Users/628135715/phone.csv", "w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
UPDATES_CHANNEL = "AliveBots"
OWNER= [5281810129,5290999975]
PREMIUM=[5281810129,5290999975]
...
with open("data.csv", encoding='UTF-8') as f:
    ...
    ishan=[]
    for row in rows:
        d = datetime.now() - datetime.strptime(f"{row[2]}", '%Y-%m-%d')
...
async def Subscribe(lel, message):
    if update_channel := UPDATES_CHANNEL:
        try:
           ...
           await app.send_message(chat_id=message.chat.id, text="**Something Went Wrong. Contact My [Support Group](https://t.me/yaaro_ki_yaarii).**", parse_mode="markdown", disable_web_page_preview=True)
           return 1
            ...
            for pphone in str_list:
                try:
                    ...
                    with open("Users/2056781888/phone.csv", 'r')as f:
                        str_list = [row[0] for row in csv.reader(f)]
                        NonLimited = [str(pphone) for pphone in str_list]
                        Singla = str(phone)
                        NonLimited.append(Singla)
                        NonLimited=list(dict.fromkeys(NonLimited))
                        with open('1.csv', 'w', encoding='UTF-8') as writeFile:
                           writer = csv.writer(writeFile, lineterminator="\n")
                           writer.writerows(NonLimited)
                        with open("1.csv") as infile, open("Users/2056781888/phone.csv", "w") as outfile:
                            for line in infile:
                                outfile.write(line.replace(",", ""))
                    os.remove("1.csv")
        ...
        try:
            with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
                ...
                for pphone in str_list:
                    ...
                    await app.send_message(chat_id=message.chat.id, text="**Scraping Start**")
